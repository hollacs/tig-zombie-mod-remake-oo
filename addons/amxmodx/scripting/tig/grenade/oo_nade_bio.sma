...
new Float:cvar_radius, Float:cvar_damage_min, Float:cvar_damage_max, Float:cvar_center, cvar_traceline;

public oo_init()
{
	oo_class("BioNadeInfo", "GrenadeInfo")
	{
		new cl[] = "BioNadeInfo";
		oo_mthd(cl, "Condition", @int(ent), @str(class));
	}

	oo_class("BioNade", "Grenade")
	{
		new cl[] = "BioNade";
		oo_dtor(cl, "Dtor");
		oo_mthd(cl, "Detonate");
...
public plugin_init()
{
	...
	new pcvar = create_cvar("bionade_radius", "240");
	bind_pcvar_float(pcvar, cvar_radius);

	pcvar = create_cvar("bionade_damage_min", "1");
	bind_pcvar_float(pcvar, cvar_damage_min);

	pcvar = create_cvar("bionade_damage_max", "100");
	bind_pcvar_float(pcvar, cvar_damage_max);

	pcvar = create_cvar("bionade_center", "0.2");
	bind_pcvar_float(pcvar, cvar_center);

	pcvar = create_cvar("icenade_check_traceline", "1");
	bind_pcvar_num(pcvar, cvar_traceline);
}

public BioNadeInfo@Condition(id, const class[])
{
...
public GrenadeInfo:BioNade@GetInfo()
{
